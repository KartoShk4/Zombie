...
const BUFF_TYPES = {
    ...
    // Негативные баффы (дебаффы)
    MOVEMENT_SLOW: 'movementSlow',      // Медленное передвижение (негативный)
    FIRE_RATE_SLOW: 'fireRateSlow',    // Медленная атака (негативный)
    POISON: 'poison',                  // Яд (урон со временем) (негативный)
    WEAKNESS: 'weakness'               // Слабость (меньше урона) (негативный)
};
...
const buffConfig = {
    ...
    [BUFF_TYPES.MOVEMENT_SPEED]: {
        ...
        color: '#4a8',
        isNegative: false,
        getDuration: () => 10,  // Фиксированная длительность 10 секунд
    ...
    [BUFF_TYPES.MOVEMENT_SLOW]: {
        name: 'Замедление',
        icon: '🐌',
        color: '#f44',  // Красный цвет для негативных баффов
        isNegative: true,
        getDuration: () => 10,  // Фиксированная длительность 10 секунд
        effect: (level) => 1 - level * 0.3  // -30% за уровень
    },
    [BUFF_TYPES.FIRE_RATE_SLOW]: {
        name: 'Медленная атака',
        icon: '⏳',
        color: '#f44',  // Красный цвет для негативных баффов
        isNegative: true,
        getDuration: () => 10,  // Фиксированная длительность 10 секунд
    ...
    [BUFF_TYPES.POISON]: {
        name: 'Яд',
        icon: '☠️',
        color: '#8f4',  // Зеленовато-красный для яда
        isNegative: true,
        getDuration: () => 10,  // Фиксированная длительность 10 секунд
        damagePerSecond: 2  // Урон в секунду
    },
    [BUFF_TYPES.WEAKNESS]: {
        name: 'Слабость',
        icon: '💔',
        ...
        isNegative: true,
        getDuration: () => 10,  // Фиксированная длительность 10 секунд
        effect: (level) => 1 - level * 0.25  // -25% урона за уровень
    }
};

// Делаем buffConfig доступным глобально для других модулей
if (typeof window !== 'undefined') {
...
function spawnBuff(buffType, x, y) {
    ...
    const newBuff = {
        ...
        x: x,
        y: y,
        size: 32,  // Увеличено до 32 для лучшей видимости
        rotation: 0,
        pulse: 0,
        lifetime: 120,  // 120 секунд на карте (увеличено)
        color: config.color,
        icon: config.icon,
        isNegative: config.isNegative || false  // Флаг негативного баффа
    };
...
function renderBuffs(ctx) {
    ...
    for (let b of buffs) {
        ...
        // Пульсация (как у монет и сердец) - проверяем настройки графики
        let pulseScale = 1;
        if (typeof getGraphicsSettings === 'function') {
            const settings = getGraphicsSettings();
            if (settings.buffPulse) {
                pulseScale = 1 + Math.sin(b.pulse) * 0.2;
            }
        } else {
            pulseScale = 1 + Math.sin(b.pulse) * 0.2;
        }
        ...
        // Определяем цвет в зависимости от типа баффа
        const isNegative = b.isNegative || false;
        const baseColor = isNegative ? '#ff4444' : b.color;  // Красный для негативных
        
        // Яркое свечение вокруг баффа (усилено для лучшей видимости)
        ctx.globalAlpha = 0.6;
        ...
        if (isNegative) {
            ctx.fillStyle = '#440000';  // Темно-красный фон для негативных
        } else {
        ...
        // Для негативных баффов добавляем красную подсветку
        if (isNegative) {
